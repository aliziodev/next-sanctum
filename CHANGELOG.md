...
- The catch-all route proxy now forwards the `User-Agent` header. Without it,
  Laravel recorded the Next server's runtime UA (`"node"`) for every request, so
  device-session lists and audit logs showed "Unknown browser" for the real
  client. (Same class as the `X-Forwarded-For` forwarding added in 0.2.0.)
...
[0.2.1]: https://github.com/aliziodev/next-sanctum/compare/v0.2.0...v0.2.1
